...
def music(request):
	...
	return render(request, "blogs/music.html", context)
@login_required
def video(request):
...
def post_detail(request, pk):
	...
	if request.method == 'POST':
		...
		form = NewCommentForm()
	return render(request, 'blogs/post_detail.html', {'post':post, 'is_liked':is_liked, 'form':form})
...
def search_posts(request):
	...
	return render(request, "blogs/search_post.html", context)

@login_required
def like(request):
	post_id = request.GET.get("likeId", '1')
	user = request.user
